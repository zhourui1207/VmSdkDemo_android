...
public:
  ...
  StreamClient(const std::string& address, unsigned port,
      std::function<void(const std::shared_ptr<StreamData>& streamDataPtr)> streamCallback,
...
private:
  ThreadPool _pool;
  std::function<void(const std::shared_ptr<StreamData>& streamDataPtr)> _streamCallback; // 码流回调，由于使用了线程池异步，所以使用智能指针来管理内存释放比较合理
