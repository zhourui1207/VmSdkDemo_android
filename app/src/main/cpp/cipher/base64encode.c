...
char base64_encode_value(uint8_t value) {
    ...
    if (value > 63) return '=';
    return base64_encode_encoding[value];
}

ptrdiff_t base64_encode_update(base64_encodestate *S, const uint8_t *data, size_t datalen, char *encoded) {
...
ptrdiff_t base64_encode(const uint8_t *data, size_t datalen, char *encoded) {
    ptrdiff_t c = 0;
