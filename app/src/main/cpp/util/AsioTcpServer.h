...
// tcp客户端session
    ...
    private:
        ...
        std::function<void(unsigned clientId, const std::shared_ptr<TcpClientSession> &sessionPtr,
                           bool isConnect)> _connectListener;
    ...
    private:
        std::function<void(unsigned clientId,
                           std::shared_ptr<PacketData> &dataPtr)> _callback; // 接收到包的回调
