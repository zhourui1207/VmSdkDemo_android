...
public:
  ...
  TcpClient(ThreadPool& pool, const std::string& address, unsigned port) :
      _tcpClient(
          std::bind(&TcpClient::receiveData, this, std::placeholders::_1, std::placeholders::_2),
          address, port), _currentStatus(AsioTcpClient::NO_CONNECT), _pool(
          pool), _isHeader(true), _readLength(Packet::HEADER_LENGTH), _connectStatusListener(nullptr) {
    // 设置状态侦听
  ...
  void receiveData(const char* pBuf, std::size_t dataLen) {
    ...
    if (callReceive) {
      ...
      if (packetPtr.get() != nullptr) {
        int usedLen = packetPtr->decode(_packetDataPtr->data(), _packetDataPtr->length());
