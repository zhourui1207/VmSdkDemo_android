...
namespace Dream {
    ...
    TcpClientSession::TcpClientSession(tcp::socket socket,
                                       ...
                                       const std::function<
                                               void(unsigned clientId,
                                                    std::shared_ptr<PacketData> &dataPtr)> &callback,
                                       const std::function<
                                               void(unsigned clientId,
                                                    const std::shared_ptr<TcpClientSession> &sessionPtr,
                                                    bool isConnect)> &connectListener,
                                       unsigned receiveSize, unsigned sendSize) :
            _clientId(0), _socket(std::move(socket)), _sessions(sessions), _callback(callback),
    ...
    void TcpClientSession::doRead() {
        ...
        _socket.async_read_some(boost::asio::buffer(_receive, _receiveSize),
                                [this, self](boost::system::error_code ec, std::size_t length) {
                                    if (!ec && length > 0) {
                                        std::shared_ptr<PacketData> dataPtr = std::make_shared<PacketData>(
                                                length, _receive);
    ...
    AsioTcpServer::AsioTcpServer(
            ...
            [](unsigned clientId, const std::shared_ptr<TcpClientSession> &,
               bool isConnect) -> void {
                printf("[%d]连接状态改变, isConnect[%d]\n", clientId, isConnect);
            }), _receive(nullptr) {
        _receive = new char[receiveSize];
    }
    ...
    void AsioTcpServer::doAccept() {
        _acceptorPtr->async_accept(*(_socketPtr.get()),
                                   [this](boost::system::error_code ec) {
                                       if (!ec) {
                                           if (!std::make_shared<TcpClientSession>(
                                                   std::move(*(_socketPtr.get())), _sessions,
                                                   _callback, _connectListener, _receiveSize,
                                                   _sendSize)->startUp()) {
