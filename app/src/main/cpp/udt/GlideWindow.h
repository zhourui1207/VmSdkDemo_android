...
namespace Dream {
    ...
    protected:
        ...
        bool continuousSeqNumber(int32_t oldSeqNumber, int32_t seqNumber) const {
            return (seqNumber == ++oldSeqNumber) ||
                   (oldSeqNumber == UDTBasePacket::SEQUENCE_NUMBER_MAX && seqNumber == 0);
        }

        int32_t increaseSeqNumber(int32_t seqNumber) const {
        ...
        int32_t increaseSeqNumber(int32_t seqNumber, int32_t offset) const {
        ...
        int32_t decreaseSeqNumber(int32_t seqNumber) const {
            if (seqNumber == 0) {
