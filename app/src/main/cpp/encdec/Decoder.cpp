...
namespace Dream {
    ...
    Decoder::Decoder()
            : _codecId(AV_CODEC_ID_H264), _inited(false), _codecContext(nullptr), _frame(nullptr),
              ...
              v_g_tab(nullptr), v_r_tab(nullptr), rgb_2_pix(nullptr), r_2_pix(nullptr),
              g_2_pix(nullptr),
              b_2_pix(nullptr) {
...
//    PrintTimer timer("DecodeNalu2YUV");
        ...
        if (_frame) {
            ...
            pgm_save2(_frame->data[1], _frame->linesize[1], _width / 2, _height / 2,
                      (uint8_t *) outUData);
            pgm_save2(_frame->data[2], _frame->linesize[2], _width / 2, _height / 2,
                      (uint8_t *) outVData);
        }
